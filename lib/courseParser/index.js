...
const GoogleTranslate = require('../googleTranslation/index');
...
class CourseParser {
  ...
  async addCourse() {
    ...
    await this.createCourseNames();
    return true;
  }

  async createCourseNames() {
    const name = this.courseDetails[0].name;
    if (!fs.existsSync(`${this.courseDir}/courseNames.json`)) {
      fs.writeFileSync(`${this.courseDir}/courseNames.json`, '');
    }
    const d = fs.readFileSync(`${this.courseDir}/courseNames.json`, '').toString();
    let allNames = '';
    if (d.length > 0) {
    ...
    fs.writeFileSync(`${this.courseDir}/courseNames.json`, JSON.stringify(allNames, null, 2));
  }
...
if (!module.parent) {
  process.umask(0);
  const parseSingleCourse = CourseParser.parseSingleCourseFlag();
  const courseFolderName = CourseParser.getCourseFolderName();
  const ifUpdateDB = CourseParser.updateDatabase();
  const ifCreateJSON = CourseParser.createJSON();
  const seeder = new CourseParser(parseSingleCourse, courseFolderName, ifUpdateDB, ifCreateJSON);
