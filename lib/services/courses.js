...
const translatedExerNames = fs.readFileSync(path.resolve(`curriculum/courseNames.json`));
  ...
  async getCourseExercise(course_id, lang, course_type, txn) {
    ...
    try {
      ...
      courseExercises[0].exercises.forEach((ele) => {
        ...
        if (JSON.parse(translatedExerNames)[newEle.name] !== undefined) {
          if (JSON.parse(translatedExerNames)[newEle.name][lang] !== undefined) {
            newEle.name = JSON.parse(translatedExerNames)[newEle.name][lang];
          }
        }
